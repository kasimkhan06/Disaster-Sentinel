...
import { CalendarToday, LocationOn, Videocam, Send } from "@mui/icons-material";
  ...
  return (
    <Card className="event-card">
      ...
      <CardContent className="event-content">
        ...
        <div className="event-details">
          ...
          {event.location_type === "online" ? (
            <>
              ...
              >
                <Videocam fontSize="small" sx={{ mr: 0.5 }} /> Online (
                {event.platform})
              </Typography>
              <Typography
                variant="body2"
        ...
        {customActions && (
          <div style={{ marginTop: "auto" }}>{customActions}</div>
        )}
      </CardContent>
    </Card>
  );
...
export default function UserAnnouncementsPage() {
  ...
  const loadCurrentUserData = async () => {
    try {
      ...
      if (
        userDataFromStorage &&
        userDataFromStorage.user_id &&
        userDataFromStorage.state &&
        userDataFromStorage.district
      ) {
        const CUser = {
  ...
  const handleRegister = async (eventId) => {
    ...
    try {
      ...
      if (!eventResponse.ok)
        throw new Error(
          `Failed to fetch event details: ${eventResponse.status}`
        );
      ...
      if (updateResponse.ok) {
        ...
        const errorData = await updateResponse
          .json()
          .catch(() => ({ detail: "Failed to update registration count" }));
        throw new Error(
          errorData.detail || "Failed to update registration count"
        );
      }
    } catch (error) {
  ...
  const sortedAnnouncements = () => {
    ...
    if (statusFilter !== "all") {
      processedAnnouncements = processedAnnouncements.filter(
        (event) =>
          event.status &&
    ...
    if (locationFilter === "online") {
      processedAnnouncements = processedAnnouncements.filter(
        (announcement) =>
          announcement.location_type &&
          announcement.location_type.toLowerCase() === "online"
      );
    } else if (locationFilter === "offline") {
      processedAnnouncements = processedAnnouncements.filter(
        (announcement) =>
          announcement.location_type &&
          announcement.location_type.toLowerCase() === "offline"
      );
    }
    if (sort === "newest") {
      processedAnnouncements.sort(
    ...
    return processedAnnouncements.filter((announcement) => {
      ...
      const eventDistrict = announcement.district
        ? announcement.district.trim().toUpperCase()
        : "";
  ...
  const filterInputLabelStyle = {
    ...
    fontStyle: "italic",
    zIndex: 1,
    pointerEvents: "none", // Add this to make the label non-interactive
  };
    ...
    <div className="background-image">
      <div className="container">
        ...
        >
          ...
          <Box sx={outerFilterBoxStyle}>
            <FormControl fullWidth>
              <InputLabel
                htmlFor="location-filter-input"
                id="location-filter-label"
                shrink
                sx={filterInputLabelStyle}
              >
                Type
              </InputLabel>
              <Select
          ...
          <Grid container spacing={3}>
            {sortedAnnouncements().map((announcement) => {
              const currentAttendees =
                Number(announcement.attendees_count) || 0;
      ...
      <Box className="footer" sx={{ mt: 4, textAlign: "center", pb: 3 }}>
        <Typography variant="body2" color="text.secondary">
          &copy; {new Date().getFullYear()} Disaster Sentinel. All rights
          reserved.
        </Typography>
      </Box>
