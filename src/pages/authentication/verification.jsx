...
const Verification = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!email || typeof email !== "string") {
      ...
      console.error(
        "OTP Verification Failed: Email is undefined or not a string."
      );
      return;
    }
  ...
  const resendOtp = async () => {
    ...
    try {
      ...
      setResendMessage(
        err.response?.data?.detail ||
          "Failed to resend OTP. Please check your email or try again later."
      );
    }
  };
  ...
  useEffect(() => {
    if (!email) {
      ...
      setError(
        "Email address not found. Please go back and try the process again."
      );
  ...
  return (
    <Box
      sx={{
        ...
        backgroundRepeat: "repeat-y", // Changed from repeat-y for better full cover
        margin: 0,
        padding: 0,
              ...
              {resendMessage && (
                <Typography
                  ...
                  color={
                    ...
                    resendMessage.startsWith("Email address is not available")
                      ? "error"
                      : "textSecondary"
                  } // Conditional color
                  mt={1}
