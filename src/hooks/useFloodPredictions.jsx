...
const useFloodPredictions = (stationData) => {
  ...
  // Fixed API request body as per requirements
  ...
  useEffect(() => {
    ...
    const fetchPredictions = async () => {
      ...
      const contextLatitude = currentStationInfo.latitude !== undefined ? currentStationInfo.latitude : DEFAULT_LATITUDE;
      const contextLongitude = currentStationInfo.longitude !== undefined ? currentStationInfo.longitude : DEFAULT_LONGITUDE;
      const contextStationId = currentStationInfo.stationId || `context-station-${String(contextLatitude).replace('.', '_')}-${String(contextLongitude).replace('.', '_')}`;

      try {
        const response = await fetch("https://disaster-sentinel-backend-26d3102ae035.herokuapp.com/api/predict/", {
          method: "POST",
